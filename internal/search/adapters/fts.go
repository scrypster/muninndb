...
// Passes the full engram text fields through so backends that need them
// (e.g. native FTS tokenization) can properly remove posting-list entries.
func (a FTSIndex) DeleteEngram(ws [8]byte, id [16]byte, concept, createdBy, content string, tags []string, createdAt int64) error {
	if a.B == nil {
		return nil
	}
	return a.B.DeleteText(context.Background(), ws, &storage.Engram{
		ID:        storage.ULID(id),
		Concept:   concept,
		CreatedBy: createdBy,
		Content:   content,
