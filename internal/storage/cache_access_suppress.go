...
// not stamp the L1 cache's recency timestamp (F4, 712-currency fix round).
//
...
// LATER, unrelated Activate() call (engine/activation/engine.go, phase6).
// A scoring pass that merely SCORES a candidate (whether or not it emits it)
// is not a user access; without this, any ReadOnly recall — COG-34's
// write-time self-query foremost among them, since it runs on every
// successful remember — could make an aged, genuinely-never-touched memory
// look "just accessed" to whatever real recall runs next, purely by having
// glanced at it.
