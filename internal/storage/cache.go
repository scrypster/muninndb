...
// engram (if present) WITHOUT updating lastAccess. F4 (712-currency fix
// round): a scoring pass is not a user access — the COG-34 write-time
// self-query (and, more generally, any ReadOnly Activate) SCORES candidates
// it never emits, and LastAccessNs (below) feeds real ACT-R recency scoring
// in a LATER, unrelated recall. Stamping on every scoring pass made an
// aged, genuinely-never-accessed memory look "just accessed" to that later
// call purely because an earlier read-only pass glanced at it. Same
// immutability contract as Get.
...
// Get/Set — stamps it. See GetNoStamp's doc for why (F4).
