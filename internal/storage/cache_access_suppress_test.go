...
// ---------------------------------------------------------------------------
// F4 (712-currency fix round): a ReadOnly-marked GetEngram/GetEngrams call
// must not stamp the L1 cache's recency timestamp (EngramLastAccessNs) —
// "a scoring pass is not a user access". Both directions matter and are
// pinned separately: advisory/read-only scoring must NOT freshen, and a
// real (non-suppressed) read still MUST freshen — otherwise the fix could
// have been "never stamp at all", which would silently break genuine
// recency-based recall for every vault.
// ---------------------------------------------------------------------------
...
func TestGetEngram_NoAccessCacheStampContext_DoesNotFreshenRecency(t *testing.T) {
	...
	// A suppressed-ctx read (the self-query's shape) must leave the cache
	// recency timestamp at 0 (not cached, in the recency sense) even after
	// reading the engram successfully.
	...
	if ns := store.EngramLastAccessNs(ws, id); ns != 0 {
		t.Fatalf("F4 violated: a suppressed-ctx GetEngram stamped cache recency (EngramLastAccessNs=%d), want 0", ns)
	}
...
func TestGetEngrams_NoAccessCacheStampContext_DoesNotFreshenRecency(t *testing.T) {
	...
	if ns := store.EngramLastAccessNs(ws, id); ns != 0 {
		t.Fatalf("F4 violated: a suppressed-ctx GetEngrams stamped cache recency (EngramLastAccessNs=%d), want 0", ns)
	}
