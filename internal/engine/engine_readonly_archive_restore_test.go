...
// ---------------------------------------------------------------------------
// F1 (712-currency fix round): Activate's phase 4.75 lazy archive restore had
// NO gate on the resolved actReq.ReadOnly (COG-11's single resolved decision).
// A read_only:true Activate — including COG-34's own similar_existing
// self-query — could therefore mint a live 0x03/0x04/0x14 association row out
// of an archived (0x25) edge, contradicting COG-11's "a read must not mutate
// learning state" text verbatim and COG-34's "writes nothing" claim.
//
// This reproduces the refuter's exact scenario: an archived edge between two
// tag-linked engrams, a ReadOnly Activate that fuses the source engram as a
// candidate, and an assertion that the live weight index stays at zero across
// the read_only call — restoring only on a subsequent NON-read-only Activate.
//
