...
func (e *Engine) activateCore(ctx context.Context, req *mbp.ActivateRequest, structuredFilter activation.EngramFilter) (*mbp.ActivateResponse, error) {
	...
	// Final valid-time gate (COG-19: default recall never returns an engram
	// whose ValidUntil <= now). Phase-6 already gated scored candidates, but
	// entity boost injects engrams that bypass passesMetaFilter — the shared
	// predicate must hold at the last cut before truncation.
