...
func (e *ActivationEngine) Run(ctx context.Context, req *ActivateRequest) (*ActivateResult, error) {
	...
	// RestoreArchivedEdgesTransitive) is exactly such a mutation. This
	// mattered before COG-34, but COG-34's write-time self-query
	// (read_only:true on every successful remember) made it fire on every
	// write. The restore still happens — lazily, on the next NON-read-only
	// Activate that reaches the edge — which is the correct semantics; a
	// read-only call must simply not be the one that performs it.
...
func (e *ActivationEngine) phase6Score(
	...
	//
	// F4 (712-currency fix round): a ReadOnly call (COG-11's single resolved
	// decision) suppresses the L1 cache recency stamp this load would
	// otherwise apply to every candidate it SCORES, not just ones it emits —
	// a scoring pass is not a user access, and EngramLastAccessNs (below)
	// feeds real recency scoring in a LATER, unrelated call. See
