...
func generateGuide(vaultName string, resolved auth.ResolvedPlasticity, stats engineStats) string {
	...
	b.WriteString("**muninn_remember tells you before you leave a rival copy.** A single (non-batch) `muninn_remember` runs the same recall pipeline against ")
	b.WriteString("the text you just wrote and returns `similar_existing`: pre-existing memories it calls a STRONG match, older than the new one by more than ")
	b.WriteString("an hour and not already linked to it by a declared `supersedes`/`contradicts` edge. It writes nothing and links nothing — it is a recall ")
	b.WriteString("run for you against the vault as your write left it, not necessarily the SAME recall you'd get by calling `muninn_recall` yourself: the ")
	b.WriteString("self-query carries no caller identity, so a candidate held by YOUR OWN active lease is hidden from it exactly as it would be from a stranger's ")
	b.WriteString("call. If one of the returned memories is what the new one replaces, `muninn_evolve` it instead of leaving both. When present, ")
	b.WriteString("`similar_existing_basis` names why the block is ABSENT rather than empty: this vault's own calibration could ")
	b.WriteString("not measure a strong band for this write (e.g. no registered embedding model) — that is \"not measured\", not \"nothing similar exists\", so ")
	b.WriteString("do not read its absence as a green light. Not run for `muninn_remember_batch`/`muninn_remember_tree`.\n")
