...
func generateGuide(vaultName string, resolved auth.ResolvedPlasticity, stats engineStats) string {
	...
	b.WriteString("an hour and not already linked to it by a declared `supersedes`/`contradicts` edge. It writes nothing and links nothing — it is the recall ")
	b.WriteString("you were already asked to do before writing, done for you. If one of them is what the new memory replaces, `muninn_evolve` it instead of ")
	b.WriteString("leaving both. When present, `similar_existing_basis` names why the block is ABSENT rather than empty: this vault's own calibration could ")
