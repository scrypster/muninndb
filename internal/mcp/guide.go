...
func generateGuide(vaultName string, resolved auth.ResolvedPlasticity, stats engineStats) string {
	...
	b.WriteString("- **muninn_update_tags** — Replace a memory's tag set IN PLACE (id, version lineage, and access history preserved). `muninn_evolve` inherits tags but cannot change them and REJECTS a `tags` argument, so to update content and tags together: evolve first, then `muninn_update_tags(new_id, tags)`. Use this for mutable tag conventions such as `due:<ISO-date>`. An empty array clears all tags, and a soft-deleted memory can still be retagged. Tags are normalized leniently, exactly as `muninn_remember` normalizes them — non-string, empty, and over-128-character entries are DROPPED (not rejected) and the set is TRUNCATED to 50 — so read the `tags` the response echoes back to see what was actually stored.\n")
