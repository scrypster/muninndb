...
func (e *slowIdempotentEngine) Evolve(ctx context.Context, vault, oldID, newContent, reason string, embedding []float32, concept string, entities []mbp.InlineEntity, effectiveAt time.Time) (*WriteResult, error) {
	return (&fakeEngine{}).Evolve(ctx, vault, oldID, newContent, reason, embedding, concept, entities, effectiveAt)
}
