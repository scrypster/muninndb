...
// computeComponents, the daysSince computation below has no unset-timestamp
// guard, and this path WRITES the result back (UpdateRelevance). If DecayWorker
// is ever wired up, a never-accessed engram would decay ~740,000 days' worth in
// one pass and the damage would be persistent, not just a bad score for one
// query. Add `if storage.IsUnsetTimestamp(c.LastAccess) { continue }` — the
// shared helper every other site now uses — before wiring.
