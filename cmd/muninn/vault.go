...
func runVaultExportMarkdown(args []string) {
	var vaultName, outputPath string
	var allVaults bool
	...
	if allVaults {
		...
		for _, name := range names {
			target := path.Join(outputPath, name+".markdown.tgz")
			if err := exportVaultMarkdownOne(name, target); err != nil {
	...
	if outputPath == "" {
		outputPath = vaultName + ".markdown.tgz"
	}
	if err := exportVaultMarkdownOne(vaultName, outputPath); err != nil {
		fmt.Printf("Error: %v\n", err)
	}
}
...
func exportVaultMarkdownOne(vaultName, outputFile string) error {
	...
	fmt.Printf("Exporting vault %q markdown to %q...\n", vaultName, outputFile)
	...
	resp, err := client.Do(req)
	if err != nil {
		return fmt.Errorf("connecting to MuninnDB: %w", err)
	}
	defer resp.Body.Close()
	...
	n, err := io.Copy(f, resp.Body)
	if err != nil {
		return fmt.Errorf("writing markdown archive: %w", err)
	}
	fmt.Printf("  Exported %d bytes to %q\n", n, outputFile)
	return nil
}
...
func listVaultNamesAdmin() ([]string, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest("GET", vaultAdminBase+"/api/vaults", nil)
	if err != nil {
		return nil, err
	}
	addSessionCookie(req)
