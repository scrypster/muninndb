...
| 0x21 | ws+engramID+fromHash(8)+PredicateHash(relType)(8)+toHash(8) | msgpack(RelationshipRecord) | relationship record — **predicate is an 8-byte hash since #894** (migration v7). The predicate component was a 1-byte discriminant from an 11-entry hardcoded map that folded every other predicate string — including four of the ten the enrich plugin's default prompt suggests — to 0xFF, so two unmapped predicates about the same entity pair collided on one key and the second write silently destroyed the first. It is now `keys.PredicateHash` of the RAW string (deliberately un-normalized: the old map was exact-match, so `"Uses"`/`"uses"` already occupied different keys, and normalizing would merge them). **The hash is never decoded back to a string** — every reader takes the predicate from the value, which carries the exact `RelType`; the only key-byte consumers copy verbatim (`RelinkRelationshipEntity`) or read the entity-hash slots (`deleteEntityLinks`). Legacy 42-byte keys are re-keyed by `internal/storage/migrate/v7_reltype_predicate_hash.go` (key length is the discriminator; undecodable values fail loud). Records already destroyed by a pre-fix collision are unrecoverable — only the survivor's bytes were on disk. In `clearVaultDataPrefixes`; still NOT in `vaultScopedExportPrefixes` — see note 7 |
