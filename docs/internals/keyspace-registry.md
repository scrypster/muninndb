...
<<<<<<< HEAD
| **0x2D** | ws+sha256(32) | engramID(16) | upsert forward-index — `idempotent_id` → engram pin (#556); relocated from 0x2B after #681/#570 allocated 0x2B/0x2C |
=======
...
allocated: 0x2B evolve-repair watermark (#681), 0x2C raw-tag-range index (S1), 0x2D upsert
forward-index (#556); 0x40–0x45 are allocated: 0x40/0x41 capability, 0x42–0x45 auth).
(`0x29`/`0x40`/`0x41` also appear in `internal/transport/mbp/frame.go` as **wire opcodes**,
a different keyspace; coincidental, safe, but confusing. Prefer `0x2E+` for new storage prefixes.)
