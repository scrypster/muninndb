...
| **0x2E** | ws+sha256(32) | engramID(16) | upsert forward-index — `idempotent_id` → engram pin (#556); relocated from 0x2D after #694 allocated ProspectiveIntent (0x2D) |
...
`0x2F`–`0x3F` and `0x46`+ are free for new storage/auth keys (0x2B, 0x2C, 0x2D, and 0x2E are now
allocated: 0x2B evolve-repair watermark (#681), 0x2C raw-tag-range index (S1), 0x2D armed-intention
index (THE PUSH #694), 0x2E upsert forward-index (#556);
